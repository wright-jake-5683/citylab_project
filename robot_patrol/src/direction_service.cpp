...
private:
  void find_safe_direction_callback(
    ...
    try {
      RCLCPP_INFO(this->get_logger(),
                  "/direction_service has been requested...");

      /*
      RCLCPP_INFO(this->get_logger(), "Request received - ranges size: %zu,
      angle_min: %.2f, angle_max: %.2f, angle_increment: %.4f",
      ...
      */
      ...
      RCLCPP_INFO(this->get_logger(),
                  "/direction_service has been completed...");
  ...
  void split_laser_data(
    ...
    /*
      RCLCPP_INFO(this->get_logger(), "angle min: %.2f",
                  request->laser_data.angle_min);
      RCLCPP_INFO(this->get_logger(), "angle max: %.2f",
                  request->laser_data.angle_max);
                  */
      ...
      for (size_t i = 0; i < request->laser_data.ranges.size(); i++) {
        ...
        if (angle >= (request->laser_data.angle_min) && angle <= -(M_PI / 6)) {
          ...
          // RCLCPP_INFO(this->get_logger(), "front i = %li", i);
                   ...
                   angle <= request->laser_data.angle_max) {
  ...
  double sum_total_dist_sec_right_() {
    ...
    for (size_t i = 0; i < total_dist_sec_right_.size(); i++) {
      // RCLCPP_INFO(this->get_logger(), "right i = %.2f",
      // total_dist_sec_right_[i]);
  ...
  double sum_total_dist_sec_left_() {
    ...
    for (size_t i = 0; i < total_dist_sec_left_.size(); i++) {
      // RCLCPP_INFO(this->get_logger(), "left i = %.2f",
      // total_dist_sec_left_[i]);
